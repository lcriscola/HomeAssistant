    ...
    public class SetCurrentThermostat : IAsyncInitializable
    {
        ...
        private void Timer_Elapsed(object? sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                var sensorAlias = String.Join(",", temperatures.Keys.Select(x => x.Alias));
                _entityManager.SetAttributesAsync(SensorId,  new
                {
                    measuredOn = DateTime.Now,
                    sensorAlias =sensorAlias,
                    sensorId = String.Join(",", temperatures.Keys.Select(x => x.SensorId)),
                    max = max.ToString("N2"),
                    min = min.ToString("N2")
                }).Wait();
                _entityManager.SetStateAsync(SensorId, average.ToString("N2")).Wait();

                _logger.LogInformation($"Setting Temperature to {average.ToString("N2")} from {sensorAlias}");
